#include "common.h"

struct PcCdLocBuffer {
    CdlLOC loc;
    u8 command_result_padding[8];
};

u8 D_8012F418[0x04];
u8 D_8012F41C[0x10];
u8 D_8012F42C[0x04];
u8 jtbl_8012F430[0x0C];
u8 D_8012F43C[0x10];
u8 D_8012F44C[0x08];
u8 jtbl_8012F454[0x04];
u8 jtbl_8012F458[0x14];
u8 D_8012F46C[0x22];
u8 pad_8012F48E[2];
void (*D_8012F490)(void);
u8 pad_8012F494[4];
DR_TPAGE D_8012F498[2];
u32 D_8012F4A8;
u32 D_8012F4AC;
u32 D_8012F4B0;
union CdSectorBuffer D_8012F4B4;
u8 D_801374B4;
u8 pad_801374B5[3];
u8 D_801374B8;
u8 pad_801374B9[3];
u8 D_801374BC[0x800];
s32 D_80137CBC;
s32 D_80137CC0;
u8* D_80137CC4;
s32 D_80137CC8;
s32 D_80137CCC;
s32* D_80137CD0;
union CdCommand D_80137CD4;
s32 D_80137CD8;
s32 D_80137CDC;
u32 D_80137CE0;
s8 D_80137CE4;
u8 pad_80137CE5[3];
u8 D_80137CE8;
u8 pad_80137CE9[3];
s32 D_80137CEC;
u8 D_80137CF0;
u8 pad_80137CF1[3];
u8 D_80137CF4;
u8 pad_80137CF5[3];
CdlLOC D_80137CF8;
RECT D_80137CFC;
struct CdCompletionSlot D_80137D04[16]; // D_80137D05/D_80137D08/D_80137D0C
u8* D_80137DC4;
s32 D_80137DC8;
u8* D_80137DCC;
s32 D_80137DD0;
u8 D_80137DD4;
u8 pad_80137DD5[3];
u8 D_80137DD8;
u8 pad_80137DD9[3];
u8 D_80137DDC;
u8 pad_80137DDD[3];
u8* D_80137DE0;
s32 D_80137DE4;
struct PcCdLocBuffer D_80137DE8;
u8 pad_80137DF4[8];
s8 D_80137DFC;
u8 pad_80137DFD[3];
u8 D_80137E00[0x4];
u8 D_80137E04[0x4];
u32* D_80137E08;
#ifdef MMX4_PC
_Alignas(void*) char D_80137E0C[SS_SEQ_TABSIZ * 3 * 10];
#else
char D_80137E0C[0x1428]; // D_80138012
#endif
s8 D_80139234[24];
u8 D_8013924C[4];
DR_MODE D_80139250[2];
SPRT D_80139268[2][17];
s32 D_80139510;
s32 D_80139514;
u8 pad_80139518[8];
CdlATV D_80139520; // spans D_80139521/22/23
u8 D_80139524;
u8 pad_80139525[3];
s8 D_80139528;
u8 pad_80139529[3];
s8 D_8013952C;
u8 pad_8013952D[3];
s32 D_80139530;
s32 D_80139534;
s32 D_80139538;
u32 D_8013953C;
s32 D_80139540;
s32 D_80139544;
u8 pad_80139548[12];
u8 D_80139554[8];
s16 D_8013955C;
u8 pad_8013955E[2];
s16 D_80139560;
u8 pad_80139562[2];
s32 D_80139564;
s8 D_80139568;
u8 pad_80139569[3];
s8 D_8013956C;
u8 pad_8013956D[11];
u8 D_80139578[0x4];
u8 D_8013957C[0x4];
u8 D_80139580[0x10];
u8 D_80139590[0x4];
u8 D_80139594[0x4];
u8 D_80139598[0x4];
u8 D_8013959C[0x4];
u8 D_801395A0[0x4];
u8 D_801395A4[0x4];
DISPENV old_dispenv[2]; // D_801395BC
u8 pad_801395D0[4];
u8 D_801395D4[0x4];
u8 D_801395D8[0x4];
u8 D_801395DC[0x4];
u8 D_801395E0[0x4];
s32 D_801395E4;
s32 D_801395E8;
u8 D_801395EC[0x4];
u8 D_801395F0[0x8];
u8 D_801395F8[0x8];
u8 D_80139600[0x10];
u8 D_80139610[0x4];
u8 D_80139614[0x4];
u8 D_80139618[0x2];
u8 D_8013961A[0x2];
u8 D_8013961C[0x2];
u8 D_8013961E[0x2];
u8 D_80139620[0x4];
u8 D_80139624[0x4];
u8 D_80139628[0x4];
u8 D_8013962C[0x4];
u8 D_80139630[0x4];
volatile s32 D_80139634;
u8 D_80139638[0x4];
u8 D_8013963C[0x8];
CdlATV D_80139644; // D_80139645/46/47
u8 D_80139648[0x5];
u8 D_8013964D[0x3];
u8 D_80139650[0x4];
u8 D_80139654[0x1C];
s32 D_80139670;
s32 D_80139674;
s32 D_80139678;
s32 D_8013967C;
s32 D_80139680;
s32 D_80139684;
s32 D_80139688;
u8 pad_8013968C[4];
struct ObjectHeader* D_80139690;
u8 pad_80139694[36];
u8 D_801396B8[0x4];
u8 D_801396BC[0x4];
u8 D_801396C0[0x4];
u8 D_801396C4[0x4];
struct DialogueGlyphData D_801396C8; // D_801396CA..D_801396CF
struct MiscObj* D_801397BC;
struct MiscObj* D_801397C0;
struct MiscObj* D_801397C4;
struct MiscObj* D_801397C8;
struct MiscObj* D_801397CC;
struct MiscObj* D_801397D0;
struct MiscObj* D_801397D4;
u8 D_801397D8;
u8 pad_801397D9[3];
struct Func80022730Config* D_801397DC;
s16 D_801397E0;
u8 pad_801397E2[2];
u16 D_801397E4[0x20];
u8 D_80139824;
u8 pad_80139825[3];
u8 D_80139828;
u8 pad_80139829[7];
struct BackgroundPrimitiveBuffer temp3[2];
struct OrderingTableBuffer temp4[2];
struct AuxiliaryPrimitiveBuffer temp5[2];
POLY_FT4 D_80139F20[2];
POLY_F4 D_80139F70[2];
DR_TPAGE D_80139FA0[2];
POLY_F4 D_80139FB0[2][128];
TILE D_8013B7B0[2];
u8 D_8013B7D0;
u8 pad_8013B7D1[7];
u8 D_8013B7D8;
u8 pad_8013B7D9[3];
u8 D_8013B7DC;
u8 pad_8013B7DD[3];
s16 D_8013B7E0;
u8 pad_8013B7E2[2];
s16 D_8013B7E4;
u8 pad_8013B7E6[2];
s16 D_8013B7E8;
u8 pad_8013B7EA[2];
s16 D_8013B7EC;
u8 pad_8013B7EE[2];
s16 D_8013B7F0;
u8 pad_8013B7F2[2];
s16 D_8013B7F4;
u8 pad_8013B7F6[2];
s16 D_8013B7F8;
u8 pad_8013B7FA[2];
s16 D_8013B7FC;
u8 pad_8013B7FE[2];
s16 D_8013B800;
u8 pad_8013B802[2];
s16 D_8013B804;
u8 pad_8013B806[2];
struct MiscObj* D_8013B808;
u8* D_8013B80C;
s8 D_8013B810;
u8 pad_8013B811[3];
u8 D_8013B814;
u8 pad_8013B815[3];
u8 D_8013B818[0x10];
struct Unk_unk68* D_8013B828;
struct Unk_unk68* D_8013B82C;
struct Unk_unk68* D_8013B830;
struct Unk_unk68* D_8013B834;
u8 jet_stingray_bubble_index;
u8 pad_8013B839[7];
u8 magma_dragoon_arena_right[0x4];
s16 magma_dragoon_arena_center[2];
u8 magma_dragoon_arena_left[0x4];
s16 magma_dragoon_arena_floor[2];
s16 magma_dragoon_arena_ceiling[4];
u16 D_8013B858[0x10];
s16 D_8013B878[0x10];
u8 D_8013B898[0x4];
u8 D_8013B89C[0x4];
u8 D_8013B8A0[0x4];
u8 D_8013B8A4[0x4];
struct ObjectHeader* D_8013B8A8;
struct EffectObj* D_8013B8AC;
struct Unk_unk68* D_8013B8B0;
void* D_8013B8B4;
u8 D_8013B8B8[0x8];
struct ShotObj* D_8013B8C0;
struct ShotObj* D_8013B8C4;
u8 D_8013B8C8[0x4];
u8 D_8013B8CC[0x4];
u8 D_8013B8D0[0xC];
u8 D_8013B8DC[0x4];
u8 D_8013B8E0[0x4];
u8 D_8013B8E4[0x4];
struct EffectObj* D_8013B8E8[22];
u16 D_8013B940[16];
u8 D_8013B960[0x10];
u8 D_8013B970[0x108];
u8 D_8013BA78[0x8];
u8 D_8013BA80[0x8];
u8 D_8013BA88[0x8];
u8 D_8013BA90[0x4];
u8 D_8013BA94[0x4];
u8 D_8013BA98[0x8];
u8 D_8013BAA0[0x4];
u8 D_8013BAA4[0x4];
u8 D_8013BAA8[0x8];
u8 D_8013BAB0[0x4];
u8 D_8013BAB4[0x4];
u8 D_8013BAB8[0x4];
u8 D_8013BABC[0x4];
u8 D_8013BAC0[0x4];
u8 D_8013BAC4[0x4];
u8 D_8013BAC8[0x4];
u8 D_8013BACC[0x4];
u8 D_8013BAD0[0x4];
u8 D_8013BAD4[0x4];
u8 D_8013BAD8[0x4];
u8 D_8013BADC[0x4];
u8 D_8013BAE0[0x14];
u8 D_8013BAF4[0x104];
u8 D_8013BBF8[0x4];
u8 D_8013BBFC[0x4];
u8 D_8013BC00[0x4];
u8 D_8013BC04[0x4];
u8 D_8013BC08[0x2];
u8 D_8013BC0A[0x2];
u8 D_8013BC0C[0x2];
u8 D_8013BC0E[0x16];
u8 D_8013BC24[0x4];
struct AbcObj abc_object;
u8 D_8013BC3C[0x4];
P_TAG* D_8013BC40[2][4][8];
u8 D_8013BD40;
u8 pad_8013BD41[3];
s32 D_8013BD44;
u16 layout_size;
u8 pad_8013BD4A[6];
s32* D_8013BD50[12][8];
struct MainObj main_objects[0x30];
void* D_8013DC10;
void* D_8013DC14;
void* D_8013DC18;
void* D_8013DC1C;
void* D_8013DC20;
void* D_8013DC24;
void* D_8013DC28;
void* D_8013DC2C;
void* D_8013DC30;
void* D_8013DC34;
void* D_8013DC38;
void* D_8013DC3C;
void* D_8013DC40;
void* D_8013DC44;
void* D_8013DC48;
void* D_8013DC4C;
u8 pad_8013DC50[4];
void* D_8013DC54;
void* D_8013DC58;
void* D_8013DC5C;
void* D_8013DC60;
void* D_8013DC64;
void* D_8013DC68;
void* D_8013DC6C;
void* D_8013DC70;
void* D_8013DC74;
void* D_8013DC78;
void* D_8013DC7C;
void* D_8013DC80;
void* D_8013DC84;
void* D_8013DC88;
void* D_8013DC8C;
void* D_8013DC90;
void* D_8013DC94;
void* D_8013DC98;
void* D_8013DC9C;
void* D_8013DCA0;
u8 pad_8013DCA4[4];
u8 D_8013DCA8[0x2];
u8 D_8013DCAA[0x2];
u8 D_8013DCAC[0x2];
u8 D_8013DCAE[0x2];
u8 D_8013DCB0[0x2];
u8 D_8013DCB2[0x2];
u8 D_8013DCB4[0x2];
u8 D_8013DCB6[0x2];
u8 D_8013DCB8[0x2];
u8 D_8013DCBA[0x2];
u8 D_8013DCBC[0x2];
u8 D_8013DCBE[0x2];
u8 D_8013DCC0[0x3];
u8 D_8013DCC3;
u8 D_8013DCC4[0x2];
u8 D_8013DCC6[0x2];
u8 D_8013DCC8[0x2];
u8 D_8013DCCA[0x2];
u8 D_8013DCCC[0x4];
u8 D_8013DCD0[0x2];
u8 D_8013DCD2[0x2];
u8 D_8013DCD4[0x2];
u8 D_8013DCD6[0x2];
u8 D_8013DCD8[0x4B0];
s32 D_8013E188[4];
s8 D_8013E198[6];
u8 pad_8013E19E[2];
u8 D_8013E1A0[0x4];
u8 D_8013E1A4[0x4];
u8 D_8013E1A8[0x2];
u8 D_8013E1AA[0xA];
u8 D_8013E1B4[0x4];
u8 D_8013E1B8[0x4];
u8 g_FilterModeB;
u8 pad_8013E1BD[1];
u16 g_FilterAmountB;
u8 D_8013E1C0[0x4];
s8 D_8013E1C4;
u8 pad_8013E1C5[3];
s8 D_8013E1C8[4];
u8 pad_8013E1CC[4];
u8 D_8013E1D0[0x18];
P_TAG* D_8013E1E8[2][4][8];
u16 cur_random;
u8 pad_8013E2EA[6];
s32 D_8013E2F0[12][8];
struct BazObj baz_objects[2];
struct VisualObj visual_objects[0x20];
u8 D_8013F310[0x18];
struct ShotObj shot_objects[0x20];
u32* D_801406A8;
u8 D_801406AC;
u8 pad_801406AD[3];
u8 D_801406B0[0x4];
u8 D_801406B4[0x4];
u8 D_801406B8[0x40];
struct WeaponObj weapon_objects[0x10];
u8 D_801410B8[0x8];
struct UnkObj unk_objects[0x14];
u8 D_80141840[0x40];
u8 D_80141880[0x8];
u8 D_80141888[0x40];
struct PlayerObj g_Player;
s32 D_801419AC;
struct BackgroundObj background_objects[3];
u8 pad_80141AAC[4];
struct UnkObj foo_objects[3];
s8 D_80141BD0;
u8 pad_80141BD1[1];
s16 D_80141BD2;
s32 D_80141BD4;
u8 pad_80141BD8[0x10]; // D_80141BD8/BDC/BDE/BDF, g_FilterModeG, g_FilterAmountG: PC main_bss_state
#ifdef MMX4_PC
u8 D_80141BE8[PC_TILEMAP_SIZE];
#else
u8 D_80141BE8[0x300];
#endif
u8* D_80141EE8[6];
u8* D_80141F00;
u16 lastFilterAmountR;
u8 pad_80141F06[2];

#ifdef VERSION_JP
u8 jp_bss_padding[0xE0];
#endif

u8 D_80141F08[0x14];
u8 D_80141F1C[0x10];
u8 D_80141F2C[0x4];
u32 D_80141F30[8];
#ifndef MMX4_PC
u8* D_80141F50[2];
u8 D_80141F58[0x10];
#endif
struct RectPtrPair* vram_rect_ptr;
u8 D_80141F6C[0x4];
u16 D_80141F70[0x800];
s32 D_80142F70;
u8 D_80142F74[0x4];
u8 D_80142F78[0x8];
struct DrawInfo* cur_draw_info;
u8* cur_draw_info_dispenv_disp_w;
u8* cur_draw_info_dispenv_screen_x;
u8* cur_draw_info_dispenv_screen_w;
u8* cur_draw_info_dispenv_isinter;
u8* cur_draw_info_drawenv;
struct EffectObj effect_objects[0x20];
u8 pad_80143598[0x18];
struct QuadObj g_QuadObjects[0x20];
u8 D_801441B0;
u8 pad_801441B1[3];
u16* D_801441B4;
u8 D_801441B8;
u8 pad_801441B9;
u16 lastFilterAmountB;
u8 D_801441BC[0x4];
u8 D_801441C0[0x8];
u16 D_801441C8[3][32][32];
union SepBundle D_801459C8;
#ifndef MMX4_PC
struct MainPrimitiveBuffer temp1[1];
#ifdef VERSION_JP
u8 pad_80153AA8[0x456C];
#else
u8 pad_801539C8[0x464C];
#endif
u8 D_80158014[0x59B4];
#ifdef VERSION_JP
u8 pad_8015D9C8[0xE0];
#endif
#endif
u8* D_8015D9C8;
#ifndef MMX4_PC
u8 pad_8015D9CC[4];
#endif
struct BgDrawRelated D_8015D9D0[2];
struct RectPtrPair vram_rect_ptrs[8];
struct ItemObj item_objects[0x20];
u8 need_palette_load;
#ifndef MMX4_PC
u8 pad_need_palette_load[3];
#endif
s32 D_80166BB4;
#ifndef MMX4_PC
u8 pad_80166BB8[0x14];
s32 _snd_openflag;
u8 pad_snd_openflag[0x14];
#endif

#ifdef MMX4_PC

u16 lastFilterAmountG;
u8 g_FilterModeG;
u16 g_FilterAmountG;

u8 D_801721B6;
u8 D_801721B8;
u8 D_801721BA;
s8 D_801721E4;

u8 D_80166D68[0x22];

u8 D_80171EA9;
u8 D_80171EA8;
struct MainObj* D_80171FE0;
u8 D_80171FE5;
s8 D_80141F4A;
u8 D_80173C84;
s32 D_80175EE8[1];
s8 D_801F6018, D_801F6019, D_801F604F;
struct DrawInfo draw_infos[2];
u16 D_80166C08;
u16 D_80166C0A;
u16 controller_state;
struct MainBssState main_bss_state;
#ifndef MMX4_PC
struct GameThread* D_801F8300;
#endif
u8* D_80173C80;
u8 pc_archive_arena[0x88000];
union TitleScratch D_80169498;
struct GameInfo game_info;
s8 D_80173C6C[4];
u8* D_80141F50[8];
struct PlayerObj g_Entity;
struct EngineObj engine_obj;
struct BarObj bar_object;
struct MiscObj misc_objects[0x40];

struct LayerObj layer_objects[4];
struct RideArmorObj qux_object;
s32 player_gfx_buf_0[0x400];
s32 player_gfx_buf_1[0x4000];
u16 g_FilterAmountR;
u8 g_FilterModeR;
u8* pc_archive_slots[22];

u8 layout_width, layout_height;
struct SecondaryPrimitiveBuffer temp2[2];
struct MainPrimitiveBuffer temp1[2];
DR_TPAGE D_80171EB0[2][6][8];
u8 window_archive_data[0x3000];
u8 replay_data[0x2000];

struct FadeState D_8016DEA0;
TILE D_80169D78[2];

#endif
